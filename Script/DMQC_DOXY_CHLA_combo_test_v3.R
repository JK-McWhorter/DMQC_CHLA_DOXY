...
for (woko in float_ids) {
  ...
  # --- PROPAGATE NPQ CORRECTION TO MAIN DATASET ---
  ...
  # Apply CHLA_NoLUT (which now has NPQ applied) to all floats, then apply physio_ratio to generate CHLA_LUT
  ...
  if (float_id %in% target_floats) {
    ...
    matched_result <- matched_result %>%
      mutate(
        CHLA_LUT     = CHLA_NoLUT, # Retains CHLA_NoLUT (including NPQ & deep zeroes)
        physio_ratio = 1
      )
  }
  ...
  matched_result_v2 <- matched_result %>%
    ...
    group_modify(~ {
      ...
      # Assign QC = 5 (value changed) for daytime observations in mixed layer where NPQ was applied
