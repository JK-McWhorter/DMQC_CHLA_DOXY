...
# Correctly converts NetCDF string/matrix arrays into numeric QC vectors matching profile levels
extract_argo_qc <- function(nc_obj, var_name, expected_length) {
  if (!var_name %in% names(nc_obj$var)) {
    return(rep(1, expected_length)) # Default to 1 (Good Data) if QC variable is absent
  }
...
for (woko in float_ids) {
  ...
  for (kiwi in crit) {
    ...
    # Extract pressure and safe PRES_QC extraction
    pres <- as.vector(ncvar_get(profile_bio, "PRES"))
    n_levels <- length(pres)
    
    if ("PRES_QC" %in% names(profile_bio$var)) {
      pres_qc <- extract_argo_qc(profile_bio, "PRES_QC", n_levels)
    } else if ("PRES_QC" %in% names(profile_physic$var)) {
    ...
    if ("TEMP" %in% names(profile_physic$var)) {
      temp_phy <- as.vector(ncvar_get(profile_physic, "TEMP"))
      pres_phy <- as.vector(ncvar_get(profile_physic, "PRES"))
      if (length(temp_phy) == length(pres)) {
        temp <- temp_phy
      } else {
        temp <- approx(x = pres_phy, y = temp_phy, xout = pres, rule = 2)$y
      }
    }
    
    if ("PSAL" %in% names(profile_physic$var)) {
      sal_phy  <- as.vector(ncvar_get(profile_physic, "PSAL"))
      pres_phy <- as.vector(ncvar_get(profile_physic, "PRES"))
      if (length(sal_phy) == length(pres)) {
        sal <- sal_phy
      } else {
        sal <- approx(x = pres_phy, y = sal_phy, xout = pres, rule = 2)$y
      }
    }
    
    # Calculate Mixed Layer Depth (MLD) using BD PRES levels
    MLD <- NA
    pres_mld <- pres
    valid_p_mask <- !(pres_qc %in% c(3, 4))
    ...
    # Extract Chlorophyll, Fluorescence, BBP700, and Oxygen using safe helper
